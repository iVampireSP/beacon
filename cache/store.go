...
// CacheStore is the cache facade over Redis — the analog of Laravel's cache store,
...
// NewCacheStore builds a cache Store (and its Locker) over the Redis client.
func NewCacheStore(client redis.UniversalClient) *CacheStore {
	return &CacheStore{client: client, locker: NewLocker(client)}
}

// Client exposes the underlying Redis client for raw/advanced use.
func (s *CacheStore) Client() redis.UniversalClient { return s.client }

// Locker exposes the distributed locker.
func (s *CacheStore) Locker() *Locker { return s.locker }

// Put stores value (JSON-encoded) under key with ttl (0 = no expiry).
func (s *CacheStore) Put(ctx context.Context, key string, value any, ttl time.Duration) error {
...
func (s *CacheStore) Forever(ctx context.Context, key string, value any) error {
	return s.Put(ctx, key, value, 0)
}

// Add stores value only if key does not exist (SET NX); reports whether stored.
func (s *CacheStore) Add(ctx context.Context, key string, value any, ttl time.Duration) (bool, error) {
	data, err := json.Marshal(value)
	if err != nil {
		return false, err
	}
...
func (s *CacheStore) Has(ctx context.Context, key string) (bool, error) {
	n, err := s.client.Exists(ctx, key).Result()
	return n > 0, err
}

// Forget deletes key.
func (s *CacheStore) Forget(ctx context.Context, key string) error {
...
// can be incremented.
func (s *CacheStore) Increment(ctx context.Context, key string, n ...int64) (int64, error) {
	return s.client.IncrBy(ctx, key, delta(n)).Result()
}

// Decrement atomically decrements key by n (default 1), returning the new value.
func (s *CacheStore) Decrement(ctx context.Context, key string, n ...int64) (int64, error) {
...
func (s *CacheStore) Flush(ctx context.Context) error {
...
func (s *CacheStore) Lock(ctx context.Context, name string, ttl time.Duration, opts *ObtainOptions) (*Lock, error) {
